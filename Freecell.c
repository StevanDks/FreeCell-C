...
void gerarBaralho(){ 
    ...
    for (i = 3; i < 7; i++){
        for (j = 1; j < 14; j++){
            if (primMonte == NULL){ //Verificando se a carta a ser gerada é a primeira do monte
...
void embaralhaBaralho(){
    int i, j, sort;
    tCarta *atual, *ant;
    for (i = 0; i < 26; i++){ //Realizar o processo de embaralhar 26 vezes (metade de 52)
        sort = rand() % 52; //Sorteando uma carta para ser embaralhada
        ant = NULL; 
        atual = primMonte;
        for (j = 0; j < sort; j++){ //Selecionando a carta a ser embaralhada
            ant = atual;
            atual = atual->prox;
        }
        if (sort > 0){  //Alterando o ponteiro do prox para "mudar a carta de lugar"
            ant->prox = atual->prox;
...
void distribuirMesa(){
    ...
    while (primMonte != NULL){ //Loop para percorer o baralho inteiro
        ...
        if (primMesa[i] == NULL){ //Verificando se a primeira posição da mesa está vazia
...
void imprime(){
    ...
    printf("[TEMP]= ");
    for (i = 0; i < 4; i++){
        if (temp[i] != NULL){ //Como todos os espaços do temp foram iniciados como NULL, se tiver alguma carta no temp ele vai printar a carta se não ele vai printar um espaço em branco
            printf("%d-[%02d|%c] ", i, temp[i]->num, temp[i]->naipe); //Printando a carta, caso tenha alguma carta na posição temp
        }else{
            printf("%d-[    ] ", i);//Printando um espaço vazio caso não tenha nenhuma carta no temp
        }
    }
    printf("\n\n");
    for (i = 0; i < 4; i++){ //For para realizar o print das 4 posições do naipe
        atual = primNaipe[i]; //Utilizando um ponteiro auxiliar para trabalhar com o primNaipe
        printf("[NAIPE %d]=", i);
        while (atual != NULL){ //Como todos os espaços do naipe foram iniciados como NULL, se tiver alguma carta no naipe ele vai printar a carta se não ele vai printar um espaço vazio ate enviarem uma carta para o naipe
            printf("[%02d|%c]", atual->num, atual->naipe); //Printando a carta, caso tenha alguma carta na posição[i] do naipe
            atual = atual->prox; //Ajustando o ponteiro para apontar para o proximo espaço vazio do primNaipe[i]
        }
        printf("\n");
    }
    printf("\n");
    for (i = 0; i < 8; i++){ //For para realizar o print das 8 posições da mesa
        atual = primMesa[i]; //Utilizando um ponteiro auxiliar para trabalhar com o primMesa
        printf("[MESA %d]=", i);
        while (atual != NULL){ //Como todos os espaços da mesa foram iniciados como NULL, se tiver alguma carta na mesa ele vai printar a carta se não ele vai printar um espaço vazio, porém apos a funcao
                               //distribuirMesa for chamada ele vai printar 7 cartas nas posições 0,1,2,3 da mesa e 6 cartas nas posições 4,5,6,7 
            printf("[%02d|%c]", atual->num, atual->naipe); //Printando a carta, caso tenha alguma carta na posição[i] da mesa
...
void moveMesaTemp(){
    ...
    scanf("%d", &posMesa);
    if ((posMesa >= 0) && (posMesa <= 7) && (primMesa[posMesa] != NULL)){ //Verificando se a posição selecionada é valida
        ...
        while (atual->prox != NULL){ //Selecionando a carta a ser movida
            ant = atual;
            atual = atual->prox;
        }
        if ((posTemp >= 0) && (posTemp <= 3) && (temp[posTemp] == NULL)){ //Verificando se a posição da temp selecionada é valida
            if (ant == NULL){
                primMesa[posMesa] = NULL; //Atualizando a ultima posição da mesa após a retirada da carta
            }
            ant->prox = NULL;
...
void moveTempMesa(){
    ...
    if ((posTemp >= 0) && (posTemp <= 3) && (temp[posTemp] != NULL)){ //Verificando se a posição da temp selecionada é valida
        ...
        scanf("%d", &posMesa);
        if ((posMesa >= 0) && (posMesa <= 7)){ //Verificando se a posição da mesa selecionada é valida
            if (primMesa[posMesa] == NULL){ //Verificando se a pilha da mesa selecionada está vazia, se estiver ele vai passar a carta sem fazer a verificação de numero e naipe
                ...
                if (atual->num - 1 == temp[posTemp]->num){ //Verificando se a jogada pode ser efetuada(verificando se o numero da carta que sai da temp pode ser posto na pilha (um exemplo de jogada que nao pode ocorrer e colocar uma carta de numero 4 em cima de uma carta de numero 2)
...
void moveMesaNaipe(){
    ...
    if((posMesa >= 0) && (posMesa <= 7) && (primMesa[posMesa] != NULL)){ //Verificando se a posição selecionada é valida
        ...
        atual = primMesa[posMesa]; //Ajustando o ponteiro auxiliar
        while((atual->prox != NULL) && (primMesa[posMesa] != NULL)){ //Selecionando a carta que vai ser movida
            ant = atual;
            atual = atual->prox;
        }
        if((posNaipe >= 0) && (posNaipe <= 3) && (((atual->num == 1) && (primNaipe[posNaipe] == NULL)) || ((primNaipe[posNaipe] != NULL) && (atual->naipe == primNaipe[posNaipe]->naipe) && (atual->num - 1 == primNaipe[posNaipe]->num)))){
            ...
            if(ant == NULL){ //Verificando se a carta que vai ser retirada da mesa é a ultima carta da pilha selecionada 
                primMesa[posMesa] = NULL; //Atualizando a mesa após a ultima carta da mesa ter sido movida
            }
            ant->prox = NULL;
            if(primNaipe[posNaipe] == NULL){ //Verificando se a posição selecionada do naipe está vazia (Primeira carta a ser adicionada no naipe)
...
void moveNaipeTemp(){
    ...
    scanf("%d", &posTemp);
    if(temp[posTemp] == NULL){ //verificando se o temp esta vazio
        if(((posNaipe >= 0) && (posNaipe <= 3) && (primNaipe[posNaipe] != NULL)) && ((posTemp >= 0) && (posTemp <= 3) && (temp[posTemp] == NULL))){ //Verifica se é um movimento valido
            if(primNaipe[posNaipe] != NULL){ //verificando se o naipe ta vazio
                if(primNaipe[posNaipe]->prox == NULL){ // caso so tenha um elemento no nape
...
void moveTempNaipe(){
    ...
    if(temp[posTemp] != NULL){
        if(((posNaipe >= 0) && (posNaipe <= 3) && (primNaipe[posNaipe] != NULL)) && ((posTemp >= 0) && (posTemp <= 3))){
...
void moveNaipeMesa(){
    ...
    printf("Posicao Pilha Mesa (0-7): "); //Selecionando a posição da mesa a ser movida(no caso a ultima carta dessa mesa vai ser movida)
    scanf("%d",&posMesa);
    if((posNaipe >= 0) && (posNaipe <= 3) && (primNaipe[posNaipe] != NULL) && ((posMesa >= 0) && (posMesa <= 7))){ //Verificando se a posição selecionada é valida
        ...
        if(((((atualMesa->naipe == 3) || (atualMesa->naipe == 4)) && ((atualNaipe->naipe == 5) || (atualNaipe->naipe == 6))) || (((atualMesa->naipe == 5) 
        || (atualMesa->naipe == 6)) && ((atualNaipe->naipe == 3) || (atualNaipe->naipe == 4)))) && (atualMesa->num > atualNaipe->num)){ //Verifica a cor do naipe e o numero da carta
            //Condicoes para as pilhas da mesa
            if(antMesa == NULL){ //Caso a mesa tenha 1 elem
                atualMesa->prox = atualNaipe;
            }
            else if (primMesa[posMesa] == NULL){ //Caso a mesa esteja vazia
                primMesa[posMesa] =  atualNaipe;
            }else{//Caso a mesa tenha mais de 1 elemento
                atualMesa->prox = atualNaipe;
            }
            //Condicoes para as pilhas dos naipes
            if(antNaipe == NULL){ //Caso a pilha naipe tenha 1 elemento
                primNaipe[posNaipe] = NULL;
            }else{ //Caso a pilha de naipes tenha mais de um elemento
                antNaipe->prox = NULL;
            }
        }else{
            printf("Movimento invalido \n");
...
int verificaFila(tCarta *aux1){
    ...
    while(aux1->prox != NULL){
        ...
        if((aux1->num == prox->num+1) &&
         (((((aux1->naipe == 3) || (aux1->naipe == 4)) && ((prox->naipe == 5) || (prox->naipe == 6))) || (((aux1->naipe == 5) 
        || (aux1->naipe == 6)) && ((prox->naipe == 3) || (prox->naipe == 4)))))){
            aux1 = aux1->prox;
        }else{
            return 0;
        }
...
void moveMesaMesa(){
    ...
    scanf("%d",&quant);
    if(((inicial >= 0) && (inicial < 8)) && ((final >= 0) && (final < 8)) && (primMesa[inicial] != NULL) && (quant >= 1)){//Verifica os parametros passado
        if(primMesa[final] == NULL){
            ...
            if(quant <= contaCarta(inicial)){
                ...
                if(verificaFila(auxI) == 1){
                    primMesa[final] = auxI; //Como estava nulo simplesmente passa toda a mesa para a nova
                    apaga = primMesa[inicial];
                    while(apaga->prox != auxI){
                         apaga = apaga->prox;
                    }
                    if(apaga == primMesa[inicial]){
            ...
            if(quant <= contaCarta(inicial)){
                ...
                if(verificaFila(auxI) == 1){   
                    if((auxF->num > auxI->num) && (((((auxF->naipe == 3) || (auxF->naipe == 4)) && ((auxI->naipe == 5) || (auxI->naipe == 6))) || (((auxF->naipe == 5) 
                    || (auxF->naipe == 6)) && ((auxI->naipe == 3) || (auxI->naipe == 4)))))){
                        auxF->prox = auxI;
                        apaga = primMesa[inicial];
                        while(apaga->prox != auxI){
                             apaga = apaga->prox;
